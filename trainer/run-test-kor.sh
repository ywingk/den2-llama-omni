...
testset="kor-AIHUB_123"

python omni_speech/infer/infer.py \
    --model-path $ROOT/expr/ft_kor/checkpoint-199000 \
    --question-file "$ROOT/test/question-${testset}.json" \
    --answer-file $ROOT/test/answer-${testset}.json \
    --num-chunks 1 \
    --chunk-idx 0 \
    --temperature 0 \
    --conv-mode llama_3 \
